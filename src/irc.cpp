...
void ThreadIRCSeed2(void* parg)
{
    ...
    while (!fShutdown)
    {
        ...
        if (fTestNet) {
            Send(hSocket, "JOIN #HHCoinTEST2\r");
            Send(hSocket, "WHO #HHCoinTEST2\r");
        } else {
            // randomly join #HHCoin00-#HHCoin05
            // int channel_number = GetRandInt(5);

            // Channel number is always 0 for initial release
            int channel_number = 0;
            Send(hSocket, strprintf("JOIN #HHCoin%02d\r", channel_number).c_str());
            Send(hSocket, strprintf("WHO #HHCoin%02d\r", channel_number).c_str());
        }
